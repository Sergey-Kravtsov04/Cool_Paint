...
{
    [Serializable]
    class OurRectangle : Figure
    {
        [NonSerialized] Pen pen;
        private Rectangle ClassRectangle = new Rectangle();
        internal OurRectangle(Point point1, Point point2, Color colorLine, Color backGroundColor, bool backGroundFill, int lineThickness) : base(point1, point2, colorLine, backGroundColor, backGroundFill, lineThickness)
        {
        }
        internal override void Draw(Graphics g, Point scrollPosition)
        {
            ...
            switch (BackgroundFill)
            {
                case true:
                    ...
                    g.FillRectangle(brush, ClassRectangle);
